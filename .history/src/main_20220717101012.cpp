...
void parseUserData(String content){
  ...
  DeserializationError deserializeJson(doc, content);
  JsonObject daily = dac["daily"];
  const char* results_0_code = doc["code"];
  const char* results_0_updateTime = doc["updatTime"];
  const char* results_0_fxLink = doc["fxLink"];
  
  const char* results_0_daily_textDay = doc["textDay"];
  const char* results_0_daily_textNight = doc["textNight"];
  const char* results_0_daily_fxData = doc["fxData"];
...
void loop() {
  ...
  String url = "https://api.qweather.com/v7/grid-weather/3d?location=116.41,39.92&key=b9fca499ed9f454ba5ccf59059c661f2";
