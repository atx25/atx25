...
void parseUserData(String content){
  ...
  JsonObject& result_0 = root["results"][0];

  const char* result_0_code = results_0["code"];
  const char* result_0_updateTime = results_0["updatTime"];
  const char* result_0_fxLink = result_0["fxLink"];
  
  JsonObject& result_0_daily = results_0["daily"];

  const char* result_0_daily_textDay = result_0_daily["textDay"];
  const char* result_0_daily_textNight = result_0_daily["textNight"];
  const char* result_0_daily_fxData = result_0["fxData"];

  Serial.println(result_0_code);                       //通过串口打印出需要的信息
  Serial.println(result_0_fxLink);
  Serial.println("白天天气"+result_0_daily_textDay);
  Serial.println("夜间天气"+result_0_daily_textNight);
  Serial.println("预报日期"+result_0_daily_fxData);
  Serial.print("\r\n");
}
void setup() {
